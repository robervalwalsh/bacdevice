...
def combinedWeather(start,end,n):
  ...
  df = {}
  for pv in points:
    cmd = f'cd /home/cleangat/daf-monitoring/bacdevice/desy-weather; /usr/bin/java -jar ArchiveReader.jar -pv krykWeather:{pv}_ai -start {start} -end {end} -method AVERAGE -count {n} -output txt -path /tmp/cleangat ; cd -'
    os.system(cmd)
    name = '/tmp/cleangat/krykWeather_'+pv+'_ai.txt'
